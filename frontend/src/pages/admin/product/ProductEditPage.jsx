...
import { getProductById, updateProduct, uploadProductImage, getAttributeNames } from '../../../services/adminApi';
...
export default function ProductEditPage() {
  ...
  useEffect(() => {
    Promise.all([getProductById(id), getCategories(), getAttributeNames()])
      .then(([prod, cats, attrs]) => {
        setForm({
          name: prod.name || '',
  ...
  if (fetching) return (
    <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-surface-500 italic">Đang tải dữ liệu sản phẩm...</p>
    </div>
  );
      ...
      <div className="flex items-center justify-between bg-surface-200 p-8 rounded-[32px] border border-white/10 shadow-2xl">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <h1 className="text-3xl font-black text-white tracking-tighter uppercase italic">Cập nhật Sản phẩm</h1>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="admin-card space-y-8 bg-surface-200/60">
            <div className="flex items-center gap-3 pb-4 border-b border-white/5">
                ...
                <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Cấu hình Hệ thống</h3>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="admin-form-label">Giá niêm yết (Gốc)</label>
                        <div className="relative">
                            ...
                            <input type="number" name="basePrice" value={form.basePrice} onChange={handleChange}
                                className="admin-form-input pl-12 font-bold text-primary-400 !bg-slate-900/50" />
                        </div>
                    </div>
                    <div>
                        <label className="admin-form-label">Thương hiệu</label>
                        <input name="brandName" value={form.brandName} onChange={handleChange}
                            className="admin-form-input !bg-slate-900/50" />
                    </div>
                </div>
          ...
          <div className="admin-card space-y-8 bg-surface-200/60 border-l-4 border-l-indigo-500">
            <div className="flex items-center justify-between pb-4 border-b border-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Thông số kỹ thuật sản phẩm</h3>
                </div>
                <button type="button" onClick={addAttribute} className="text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:text-white flex items-center gap-2 transition-colors">
                    <PlusCircle className="w-4 h-4" /> Thêm tiêu chí
                </button>
            </div>
          ...
          <div className="admin-card space-y-8 bg-surface-200/60">
            <div className="flex items-center justify-between pb-4 border-b border-white/5">
                ...
                <button type="button" onClick={addVariant} className="text-[10px] font-black text-amber-400 uppercase tracking-widest hover:text-white flex items-center gap-2 transition-colors">
                    <Plus className="w-4 h-4" /> Thêm SKU
                </button>
            </div>
                ...
                {form.variants.map((v, idx) => (
                    <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-4 p-6 bg-slate-900/40 rounded-[24px] border border-white/5">
                        <div className="md:col-span-4 space-y-2">
                            <label className="text-[9px] font-black text-surface-500 uppercase tracking-widest ml-2">Mã SKU</label>
                            <input value={v.sku} onChange={e => updateVariant(idx, 'sku', e.target.value)} required
                                className="admin-form-input !py-3 !bg-slate-800 font-mono" />
                        </div>
                        <div className="md:col-span-4 space-y-2">
                            ...
                            <input type="number" value={v.price} onChange={e => updateVariant(idx, 'price', e.target.value)} required
                                className="admin-form-input !py-3 font-bold text-primary-400 !bg-slate-800" />
                        </div>
                        <div className="md:col-span-3 space-y-2">
                            <label className="text-[9px] font-black text-surface-500 uppercase tracking-widest ml-2">Tồn kho</label>
                            <input type="number" value={v.stock} onChange={e => updateVariant(idx, 'stock', e.target.value)} required
                                className="admin-form-input !py-3 text-center !bg-slate-800" />
                        </div>
            ...
            <div className="admin-card bg-surface-200/60 sticky top-8 space-y-8">
                <div className="space-y-6">
                    <div className="flex items-center gap-3 pb-4 border-b border-white/5">
                        ...
                        <h3 className="text-xs font-black text-white uppercase tracking-[0.2em]">Thư viện Ảnh</h3>
                    </div>
                    ...
                    <label className="relative block group cursor-pointer">
                        <div className="flex flex-col items-center justify-center py-10 px-4 border-2 border-dashed border-white/10 rounded-[24px] bg-slate-900/50 hover:bg-primary-600/5 hover:border-primary-500/50 transition-all">
                            ...
                            <p className="text-[9px] font-black text-white uppercase tracking-widest">Thay đổi ảnh</p>
                        </div>
                ...
                <div className="pt-4 space-y-4">
                    <button type="submit" disabled={loading}
                        ...
                        <span className="text-sm font-black">{loading ? 'Processing...' : 'Lưu Thay đổi'}</span>
                    </button>
